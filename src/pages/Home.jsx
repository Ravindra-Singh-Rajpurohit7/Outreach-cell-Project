...
import { 
  ...
  Zap, Rocket, CheckCircle, Plus, X, Clock, AlertCircle 
} from 'lucide-react';
...
export default function Home() {
  ...
  // Deadlines Data (According to Outreach Cell requirements)
  const deadlines = [
    { id: 1, task: "Summer Internship Reg.", date: "15 April", urgent: true },
    { id: 2, task: "DORA Scholarship App", date: "22 April", urgent: false },
    { id: 3, task: "Cognizance Volunteer", date: "12 April", urgent: true },
  ];
    ...
    <div className="max-w-7xl mx-auto p-6 md:p-12 relative font-sans">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, y: 50 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 50 }}
              ...
              <div className="flex justify-between items-center mb-8 border-b pb-6 text-gray-900">
                ...
                <button onClick={() => setIsModalOpen(false)} className="p-3 bg-gray-50 rounded-2xl text-gray-900"><X size={20} /></button>
              </div>
              <form onSubmit={handleAddPost} className="space-y-6">
                <input required placeholder="Headline..." className="w-full p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-900" 
                  onChange={(e)=>setNewPost({...newPost, title: e.target.value})} />
                <textarea required placeholder="Short Summary..." className="w-full p-4 bg-gray-50 rounded-2xl outline-none min-h-[120px] text-gray-900" 
                  onChange={(e)=>setNewPost({...newPost, excerpt: e.target.value})} />
                <select className="w-full p-4 bg-gray-50 rounded-2xl outline-none font-bold uppercase text-xs tracking-widest text-gray-900"
      ...
      <header className="mb-16 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        ...
        <div className="relative w-full lg:w-96">
          ...
          <input className="w-full pl-12 pr-6 py-4 bg-white border border-gray-100 rounded-2xl shadow-sm outline-none text-gray-900" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              ...
              <motion.div layout key={post.id}>
                <Link to={`/post/${post.id}`} className="group block bg-white border border-gray-50 rounded-[3rem] shadow-sm hover:shadow-2xl transition-all p-4">
                  ...
                  <div className="px-6 pb-6">
                    ...
                    <h2 className="text-3xl font-black my-4 group-hover:text-blue-600 transition-colors leading-tight text-gray-900">{post.title}</h2>
        ...
        <aside className="space-y-8">
           {/* IMPORTANT DEADLINES (New Feature based on Outreach Statement) */}
           <div className="bg-white p-8 rounded-[3rem] border border-gray-50 shadow-sm">
             <h3 className="text-xl font-black mb-6 flex items-center gap-2 text-gray-900 underline decoration-blue-500">
               <Clock size={22} className="text-blue-600" /> Key Deadlines
             </h3>
             <div className="space-y-4">
               {deadlines.map(item => (
                 <div key={item.id} className={`p-4 rounded-2xl border ${item.urgent ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100'}`}>
                   <div className="flex justify-between items-start">
                     <p className={`font-bold text-sm ${item.urgent ? 'text-red-700' : 'text-gray-700'}`}>{item.task}</p>
                     {item.urgent && <AlertCircle size={14} className="text-red-500" />}
                   </div>
                   <p className="text-[10px] font-black text-gray-400 mt-1 uppercase tracking-widest">{item.date}</p>
                 </div>
               ))}
             </div>
           </div>
             ...
             {subscribed ? (
               ...
               <>
                 ...
                 <form onSubmit={handleSubscribe} className="space-y-3">
                   <input required type="email" placeholder="email@iitr.ac.in" className="w-full p-4 bg-gray-800 rounded-2xl outline-none text-white focus:ring-1 focus:ring-blue-500" />
